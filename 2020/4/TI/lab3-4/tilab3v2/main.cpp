...
symb* readAlphabet(int n, istream& in) {
    symb* a = new symb[n];
    cout << QString::fromUtf8("Введите символы и их вероятности").toLocal8Bit().data() << endl;
    for (int i = 0; i < n; i++) {
        in >> a[i].s >> a[i].prob;
        a[i].top = -1;
    }
...
void printAlphabet(symb* a, int n) {
    for (int i = n - 1; i >= 0; i--) {
        cout << a[i].s <<QString::fromUtf8(" Вероятность:").toLocal8Bit().data() << a[i].prob << " \t";
        cout << QString::fromUtf8("Ш-Ф: ").toLocal8Bit().data() << a[i].codeSH << " \t";
...
void averageLen(symb * a, int n, int blockSize) {
    ...
    cout << QString::fromUtf8("По Шеннону-Фано: ").toLocal8Bit().data() << maxlen1/blockSize;
    cout << QString::fromUtf8(", по Хаффману: ").toLocal8Bit().data() << maxlen2/blockSize << endl;
}
...
string encode(symb* a, int n, string msg, int blockSize, bool isH) {
    ...
    cout << QString::fromUtf8("Добавим к сообщению 3 бита - размер блока: ").toLocal8Bit().data() << z << endl;
    prefix += z.to_string();
    prefix += " ";

    cout << QString::fromUtf8("Разобъем сообщение на блоки:").toLocal8Bit().data() << endl;
    ...
    if (msg.length() % blockSize) {
        cout << QString::fromUtf8("Последний блок не полный, добавим к нему ").toLocal8Bit().data() << msg.length() % blockSize << QString::fromUtf8(" символ(ов).").toLocal8Bit().data() << endl;
        cout << QString::fromUtf8("Также добавим к сообщению количество символов, которые нужно будет отбросить при декодировании:").toLocal8Bit().data();
...
int main() {
    ...
    ifstream in;
    char md;
    cout << QString::fromUtf8("Ввести с файла? (0 - нет, 1 - да): ").toLocal8Bit().data();
    cin >> md;
    symb* a;
    if (md == '1') {
        in.open("inp.txt");
        ...
        cout << QString::fromUtf8("Введите количество символов").toLocal8Bit().data() << endl;
        cin >> n;
    ...
    //printAlphabet(a, n);
    cout << QString::fromUtf8("Энтропия источника:").toLocal8Bit().data() << entropy(a, n) << endl;
    //cout << "Средняя длинна кода:"  << endl;
    //averageLen(a, n, 1);

    string inp;
    cout << QString::fromUtf8("Введите сообщение, которое нужно закодировать: ").toLocal8Bit().data() ;
    cin >> inp;
    cout  << inp.length() << QString::fromUtf8(" - длина введенного сообщения").toLocal8Bit().data() << endl;
    cout << QString::fromUtf8("При кодировании использовать коды Хаффмана(1) или Шеннона-Фано(0)?: ").toLocal8Bit().data();
    bool isH;
    cin >> isH;

    cout << QString::fromUtf8("Использовать поблочное кодирование? (0 - нет, 1 - да): ").toLocal8Bit().data();
    int mode;
    cin >> mode;

    if (mode) {
        int blockSize;
        cout << QString::fromUtf8("Введите длину блоков: ").toLocal8Bit().data();
        cin >> blockSize;
        if (blockSize > 4) {
            cout << QString::fromUtf8("С данной длиной блока вычисление кодов будет длиться долго. Продолжить? 0/1").toLocal8Bit().data() << endl;
            cin >> mode;
            if (mode == 0) exit(2);
        }
        ...
        cout << QString::fromUtf8("Результат кодирования сообщения ").toLocal8Bit().data() << inp << ":" << endl;
        cout << resultStr << endl;
        cout << QString::fromUtf8("Вывести алфавит? (0 - нет, 1 - да) ").toLocal8Bit().data();
        char c;
        cin >> c;
        if (c == '1'){
            printAlphabet(res, resn);
