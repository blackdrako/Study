...
void Symbol::print(){
    cout<<"\""<<this -> symb<<"\" - "<<this->prob<<endl;
};

void Symbol::print_code(){
    cout<<"\""<<this -> symb<<"\" - "<<this->prob <<" - "<< this->code<<endl;
};
...
void Alphabet::read(){
    cout << QString::fromUtf8("Введите алфавит размера ").toLocal8Bit().data() << this->size << " :\n";
    string t_symb;
    ...
    while(input_flag){
        ...
        for(unsigned long i=0;i<this->size;i++){
            ...
            else cout << QString::fromUtf8("Сумма вероятностей появления символов не равна 1. Повторите ввод.").toLocal8Bit().data() << endl;
        #endif
    }
...
void Alphabet::print_codes(){
    ...
    cout << QString::fromUtf8("Средняя длина кода: ").toLocal8Bit().data() << this->avg_len() << endl << endl;
};
...
double Alphabet::sum_probs(int begin,int end){
 ...
 double Alphabet::avg_len(){
     double result=0;
     double count=0.0;
     for(auto i=this->arr.begin();i!=this->arr.end();i++){
        result+=i->get_code().length()*i->get_prob();
        count++;
     }
    result =result/(this->arr[0].get_symb().length());
    return result;
 };
...
void Alphabet::build_code_arr(int mode){
    ...
    switch (mode){
        case SHENNON_FANO_CODE:
            cout<< endl << QString::fromUtf8("Построение кода Шеннона - Фано.").toLocal8Bit().data();
            this->shennoncode_build_step(0,this->size);
            break;
        break;
        case HUFFMAN_CODE:
            cout << endl << QString::fromUtf8("Построение кода Хаффмана.").toLocal8Bit().data();
            this->getHuffmanCodes();
            break;
        default:
            cout << QString::fromUtf8("НЕКОРЕКТНЫЙ ВЫБОР АЛГОРИТМА ПОСТРОЕНИЯ ОПТИМАЛЬНОГО КОДА").toLocal8Bit().data() << endl << QString::fromUtf8("ИСПРАВЬТЕ ИСХОДНЫЙ КОД").toLocal8Bit().data() << endl;
            exit(1);
        break;
    };
};
...
string Alphabet::encode_text(string str){
    ...
    unsigned addition_symbols = block_size - (str.length()%block_size);
        ...
        cout<<QString::fromUtf8("Первый байт закодированного сообщения, содержит размер блока - ").toLocal8Bit().data()<<block_size<<endl;
        cout<<QString::fromUtf8("Второй байт закодированного сообщения, содержащий кол-во добавленных символов - ").toLocal8Bit().data()<<addition_symbols<<endl;
    ...
    for(unsigned i=0;i<addition_symbols;i++){
        str+=this->arr[0].get_symb();
        cout<<QString::fromUtf8("Дописываем в последний блок ").toLocal8Bit().data()<<arr[0].get_symb()[0]<<endl;
    }

    //Вычисляем размер текста в блоках.
    unsigned str_block_count=str.length()/block_size;

    cout<<QString::fromUtf8("Сообщение после дописывания символов в последний блок - ").toLocal8Bit().data();
    for(int i=0;i<str_block_count;i++){
            cout<<str.substr(i*block_size,block_size)<<" ";
    };
    cout<<endl;

    cout<<QString::fromUtf8("Закодированное сообщение - ").toLocal8Bit().data()<<num_to_binstr(block_size)<<" "<<num_to_binstr(addition_symbols)<<" ";
    for(unsigned i=0;i<str_block_count;i++){
        ...
        if(flag){
            cout<<QString::fromUtf8("	Неверно введено слово. Завершение программы.").toLocal8Bit().data()<<endl;
            exit(1);
        }
        else {
            result_code+=this->arr[symbol_index].get_code();
            cout<<this->arr[symbol_index].get_code()<<" ";
        };
    };
    cout<<endl;
    return result_code;
