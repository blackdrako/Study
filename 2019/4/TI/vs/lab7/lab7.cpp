...
*/
/*
 * UX/UI запрашивает у пользователя s, получает ответ между min и max
 */
...
int main() {
    /* Windows-specific */
    system("chcp 65001"); // Используем UTF-8
    system("cls");
	int count = 0;
    // Инициализируем ГПСЧ
    srand(time(NULL));
	...
	while (!exit_f) {
		...
		//int c = choose("Выберите применяемый код: 1 - (3; 1), 2 - (7; 4)", 1, 2) - 1;
        ...
        while(it + 4 <= raw_str.end()) {
           ...
           // std::cout << y.str_epsilon() << " = ";
			...
			std::cout << "\tРез.: i(x)=" << y.str() << std::endl;// << "=" << y.str_bit() << std::endl;
