...
long SimplexArtificial::simplexArtificialStep()
{
    ...
    if (max == 0) return 0;   //ñèìïëåêñ-òàáëèöà ÿâëÿåòñÿ ïîñëåäíåé
    j = t;
    for (i = 0; i < x1 - 1; i++)
        if (T_y[i][j] > 0) {
            min = T_y[i][0] / T_y[i][j];
...
void SimplexArtificial::replace_y() {
    ...
    for (i = 0; i < x1 - 1; i++) {
        ...
        std::cout << "Èñêëþ÷åíèå y èç ðàçðÿäà áàçèñíûõ:" << std::endl;
...
long SimplexArtificial::simplexArtificial() {
    ...
    long *a = new long[n - 1];	//ìàññèâ óåäèí¸ííûõ ïîëîæèòåëüíûõ êîýôôèöèåíòîâ (áàçèñíûõ ñòîëáöîâ)
    for (long j = 0; j < n - 1; j++) {
        a[j] = -1;
        for (long i = 0; i < m; i++) {
            if (SLU[i][j] < 0) {
